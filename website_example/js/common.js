 ...
 * Author: Muscleman
 * GitHub: https://github.com/muscleman/cryptonote-nodejs-pool
 **/
...
function formatDate (time) {
    ...
    return new Intl.DateTimeFormat('en-US', options).format(new Date(parseInt(time) * 1000))
}
...
function getReadableCoin (stats, coins, digits, withoutSymbol) {
    ...
    let amount = parseFloat((coins || 0) / stats.config.coinUnits)
...
function formatLuck (difficulty, shares, solo = false) {
    ...
    if (lastStats.config.slushMiningEnabled) {
        ...
        accurateShares = 1 / lastStats.config.blockTime * ( // 1/blockTime to get the average
    ...
    let percent = 0.00
    if (difficulty > 0)
        percent = parseFloat(accurateShares / difficulty * 100).toFixed(2)
    if (!percent) {
        return `<span class="luckGood">0.00%&nbsp;</span>` + (solo === true ? `<span class="fa fa-user luckGood" title="Solo Mined"></span>` : ``);
...
function getTransactionUrl (id, stats) {
    if (stats && blockExplorers && blockExplorers[stats.config.coin]) {
...
function poolBlocks_updateStats(stats, key) {
    updateText(`blocksTotal${key}`, stats.pool.totalBlocks.toString());
    ...
    updateText(`blocksTotalSolo${key}`, stats.pool.totalBlocksSolo.toString());
    if (stats.pool.lastBlockFoundSolo) {
        var d = new Date(parseInt(stats.pool.lastBlockFoundSolo))
            .toISOString();
    ...
    updateText(`blocksMaturityCount${key}`, stats.config.depth.toString());
    $(`#averageLuck${key}`)
        .html(formatLuck(stats.pool.totalDiff, stats.pool.totalShares));
    $(`#averageLuckSolo${key}`)
        .html(formatLuck(stats.pool.totalDiffSolo, stats.pool.totalSharesSolo));
}
...
function poolBlocks_InitTemplate (ranOnce, displayedChart, xhrGetBlocks) {
    ...
    poolBlocks_updateStats(lastStats, coin)    
/*
    ...
    $(`#averageLuck${coin}`)
        .html(`PROP: ${formatLuck(lastStats.pool.totalDiff, lastStats.pool.totalShares)}`);
    $(`#averageLuckSolo${coin}`)
        .html(`SOLO: ${formatLuck(lastStats.pool.totalDiffSolo, lastStats.pool.totalSharesSolo)}`);
*/
...
/*
            ...
            $(`#averageLuckSolo${key}`)
                .html(formatLuck(mergedStats[key].pool.totalDiffSolo, mergedStats[key].pool.totalSharesSolo));
*/
 ...
 **/
 ...
 let workerstats_graphSettings = {
    ...
    tooltipFormat: '<div class="jqsfield"><span style="color:{{color}}">{{y}}</span> &ndash; {{offset:names}}<br></div>',
...
function workerstats_GetWorkerCells (worker) {
    ...
    return '<td class="col1" data-sort="' + status + '"><i class="fa fa-' + (status == 'ok' ? 'plug status-ok' : 'times status-error') + '"></i></td>' +
...
function workerstats_GetBlockRewardCells (block_reward, stats) {
    function formatBlockRewardLink (block_reward, stats) {
        return '<a target="_blank" href="' + getBlockchainUrl(block_reward.hash, stats) + '">' + block_reward.height + '</a>';
    }
    return `<td class="col1">${formatBlockRewardLink(block_reward, stats)}</td>` +
        `<td class="col2">${formatDate(block_reward.time)}</td>` +
        `<td class="col3">${block_reward.difficulty}</td>` +
        `<td class="col4">${formatLuck(block_reward.difficulty, block_reward.shares, false)}</td>` + 
        `<td class="col5">${getReadableCoin(stats, block_reward.blockReward)}</td>` +
        `<td class="col6">${getReadableCoin(stats, block_reward.reward)} ( = ${(block_reward.percent * 100).toFixed(2)}% after pool fees)</td>` +
        `<td class="col7"><i class="fa fa-${(block_reward.rewardType === 'prop' ? 'users' : 'user')} status-ok"></i> ${block_reward.rewardType}</td>`;
}
