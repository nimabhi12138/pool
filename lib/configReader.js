...
// Allow overriding Redis connection via environment variables for containerized setups
if (global.config && global.config.redis) {
    if (process.env.POOL_REDIS_HOST)
        global.config.redis.host = process.env.POOL_REDIS_HOST;
    if (process.env.POOL_REDIS_PORT)
        global.config.redis.port = parseInt(process.env.POOL_REDIS_PORT, 10);
    if (Object.prototype.hasOwnProperty.call(process.env, 'POOL_REDIS_AUTH'))
        global.config.redis.auth = process.env.POOL_REDIS_AUTH || null;
    if (process.env.POOL_REDIS_DB)
        global.config.redis.db = parseInt(process.env.POOL_REDIS_DB, 10);
}

if (global.config && global.config.daemon) {
    if (process.env.POOL_DAEMON_HOST)
        global.config.daemon.host = process.env.POOL_DAEMON_HOST;
    if (process.env.POOL_DAEMON_PORT)
        global.config.daemon.port = parseInt(process.env.POOL_DAEMON_PORT, 10);
}

if (global.config && global.config.wallet) {
    if (process.env.POOL_WALLET_HOST)
        global.config.wallet.host = process.env.POOL_WALLET_HOST;
    if (process.env.POOL_WALLET_PORT)
        global.config.wallet.port = parseInt(process.env.POOL_WALLET_PORT, 10);
}

if (global.config && global.config.poolServer && process.env.POOL_ADDRESS) {
    global.config.poolServer.poolAddress = process.env.POOL_ADDRESS;
}
